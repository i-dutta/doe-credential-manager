...
with tabs[0]:
    ...
    if df.empty:
        ...
        headers = ["Env", "Application", "URL", "Username", "Password", "✏️", "🗑️"]
        ...
        for i, row in df.iterrows():
            ...
            if st.session_state.get(pwd_key, False):
                ...
                if cols[4].button("🙈 Hide", key=f"hide_btn_{i}"):
                    st.session_state[pwd_key] = False
                    st.rerun()
            else:
                masked_password = "•" * len(str(row["Password"])) if row["Password"] else ""
                cols[4].write(masked_password)
                if cols[4].button("👁 Show", key=f"show_btn_{i}"):
